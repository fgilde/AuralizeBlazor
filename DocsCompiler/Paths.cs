...
namespace MudBlazor.Docs.Compiler
{
    public class Paths
    {
        private const string DocsDirectory = "AuralizeBlazor.Sample";
        ...
        public static string SrcDirPath
        {
            get
            {
                ...
                Console.WriteLine("WP " + workingPath);

                do
                {
                    workingPath = Path.GetDirectoryName(workingPath);
                        Console.WriteLine("WP NEW " + workingPath);

                }
                while (Path.GetFileName(workingPath) != "Sample" && !string.IsNullOrWhiteSpace(workingPath));
